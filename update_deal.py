...
NUMERO_OFFERTE = 10  # ultime offerte Telegram mostrate in home
...
def estrai_ultime_offerte(numero_offerte=NUMERO_OFFERTE):
    ...
    # Telegram può contenere markup/ripetizioni nella pagina pubblica.
    # Usiamo il link al post come identificatore stabile e prendiamo
    # esattamente le ultime offerte uniche disponibili.
    for messaggio in reversed(messaggi):
        dati = analizza_messaggio(messaggio)
        if not e_unofferta_valida(dati):
            continue

        link_post = dati.get("link_telegram_post", "").strip()
        if not link_post or link_post in visti:
            continue
