...
def cerca_categoria(token, parola_chiave, numero_risultati=8):
    headers = {
...
def normalizza_risultati(dati_grezzi):
    ...
    for el in elementi:
        try:
            titolo = (
                el.get("itemInfo", {}).get("title", {}).get("displayValue")
                or el.get("title", "")
            )
            immagine = (
                el.get("images", {}).get("primary", {}).get("medium", {}).get("url")
                or el.get("imageUrl", "")
            )
            ...
            prezzo_scontato = (
                (listino.get("price") or {}).get("displayAmount")
                or (listino.get("price") or {}).get("amount", "")
            )
            prezzo_originale = (listino.get("savingBasis") or {}).get("displayAmount", "")

            sconto = ""
            risparmio = listino.get("saving") or listino.get("savings") or {}
            if risparmio:
                valore = risparmio.get("percentage") or risparmio.get("percent")
                if valore:
                    sconto = f"{valore}%"

            link = el.get("detailPageURL") or el.get("detailPageUrl", "")
